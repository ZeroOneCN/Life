...
export function buildForexDashboardSummary(
  ...
  const totalDeposit = roundMoney(scopedCapital
  ...
  // 体验金入金不计入净值；体验金失效（bonus_expired）在 MT5 中转为真实余额，计入入金
  const allTotalDeposit = roundMoney(capitalFlows
    .filter((flow) => (flow.flowType === 'deposit' && !flow.isBonus) || flow.flowType === 'bonus_expired')
    .reduce((sum, flow) => sum + flow.amount, 0));
  ...
  return {
    ...
    netCapital,
    equity,
    roi: totalDeposit > 0 ? equity / totalDeposit : 0,
  };
}
...
export function computeForexMultiPosition(
  ...
  /* 账户级爆仓：总保证金 × 强平比例 = 爆仓时必须保留的最小净值 */
  const liquidationEquityThreshold = roundMoney(totalMargin * forcedLiquidationRatio);
...
function parseForexImportInstrument(value: unknown): ForexInstrument {
  ...
  // 2. 去掉末尾的非字母数字字符（处理 XAUUSD+, XAUUSD# 等）
  const withoutSymbols = withoutDot.replace(/[^A-Za-z0-9]+$/, '');

  // 3. 去掉末尾连续的小写字母后缀（处理 XAUUSDm, EURUSDecn 等）
  const suffixMatch = withoutSymbols.match(/^([A-Z]+[A-Z0-9]*)([a-z]+)$/);
  const withoutSuffix = suffixMatch ? suffixMatch[1] : withoutSymbols;
