...
export default function FinanceReportPage({ embedded = false }: { embedded?: boolean }) {
  ...
  const yearOptions = useMemo(() => {
    ...
    return [current, current - 1, current - 2, current - 3, current - 4].map((value) => ({
      value,
      label: `${value} 年`,
    }));
  }, []);
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      const channelSummary = logs
        .map(
          (item) =>
            `${channelLabels[item.channel] ?? item.channel}:${channelStatusLabels[item.status]}`,
        )
        .join(' / ');

      if (logs.length === 0) {
        showToast(
          '月报已生成，但通知场景尚未配置渠道，请在通知中心 > 场景绑定添加渠道。',
          'warning',
        );
      } else if (successCount === 0) {
        showToast(
          `月报推送未真正下发：${channelSummary || '无渠道可用'}。请检查通知中心的渠道配置。`,
          'error',
        );
      } else {
        const detail = channelSummary ? `，渠道：${channelSummary}` : '';
        showToast(
          `月报已远程推送到 ${successCount} 个渠道（跳过 ${skippedCount} / 失败 ${errorCount}）${detail}`,
          'success',
        );
      }
    } catch (error) {
  ...
  const monthlyStats = useMemo(() => {
    ...
    return [
      ...
      {
        label: '环比上月',
        value: formatChange(report.monthOverMonthChange, report.monthOverMonthChangePercent),
        helper: `上月 ${formatCurrency(report.previousMonthExpense)}`,
        tone: momTone,
      },
      {
        label: '同比去年',
        value: formatChange(report.yearOverYearChange, report.yearOverYearChangePercent),
        helper: `去年同月 ${formatCurrency(report.lastYearSameMonthExpense)}`,
        tone: yoyTone,
      },
      {
        label: '覆盖模块',
        value: `${report.moduleBreakdown.filter((item) => item.count > 0).length} / 5`,
        helper: report.investment?.tradeCount
          ? `投资 ${report.investment.tradeCount} 笔`
          : `${report.topExpenses.length} 笔 Top3`,
      },
    ];
  }, [report]);
      ...
      <Row gutter={[24, 20]}>
        ...
        <Col span={24}>
          <SectionCard
            ...
            action={
              <Btn tone="primary" onClick={handleLoadAiSummary} disabled={aiLoading || !report}>
            ...
            {aiLoading ? (
              ...
              <div className="finance-report-ai">
                {aiSummary.enabled ? (
                  ...
                  <EmptyState title="AI 摘要未启用" description={aiSummary.summary} icon="⚙️" />
                )}
              </div>
            ) : (
              <EmptyState
          ...
          <Col span={24}>
            <SectionCard
              ...
              action={
                <Tag
                  tone={
                    report.investment.exchangeRateSource === 'exchangerate-api' ? 'blue' : 'default'
                  }
                >
                  {report.investment.exchangeRateSource === 'exchangerate-api'
                    ? '实时汇率'
                    : '降级汇率'}
                </Tag>
              }
            >
              <div className="callout callout-info" style={{ marginBottom: 12 }}>
                汇率 1 {report.investment.currency} = {report.investment.exchangeRate.toFixed(4)}{' '}
                CNY （
                {report.investment.exchangeRateSource === 'exchangerate-api'
              ...
              <StatGrid
                items={[
                  ...
                  {
                    label: '手续费',
                    value: formatUsd(report.investment.totalCommission),
                    helper: `隔夜费 ${formatUsd(report.investment.totalOvernightFee)}`,
                  },
                ]}
              />
                ...
                <DataTable
                  ...
                  columns={[
                    { key: 'instrument', title: '品种', render: (_, row) => row.instrument },
                    {
                      ...
                      title: '净收益',
                      align: 'right' as const,
                      render: (_, row) => `${row.netPnl >= 0 ? '+' : ''}${formatUsd(row.netPnl)}`,
                    },
                    {
                      key: 'tradeCount',
                      title: '笔数',
                      align: 'right' as const,
                      render: (_, row) => `${row.tradeCount}`,
                    },
                    {
                      key: 'commission',
                      title: '手续费',
                      align: 'right' as const,
                      render: (_, row) => formatUsd(row.commission),
                    },
                  ]}
                />
              )}
          ...
          <Col span={24}>
            <SectionCard
              ...
              action={
                <Tag
                  tone={
                    report.netWorth.exchangeRateSource === 'exchangerate-api' ? 'blue' : 'default'
                  }
                >
              ...
              <div className="finance-report-year">
                <StatGrid
                  items={[
                    ...
                    {
                      label: '覆盖月份',
                      value: `${yearReport.months.filter((item) => item.total > 0).length} / 12`,
                    },
                    {
                      label: '月均支出',
                    ...
                    },
                    {
                      ...
                      value: formatCurrency(
                        Math.max(...yearReport.months.map((item) => item.total)),
                      ),
                      helper: yearReport.months.length
                        ? yearReport.months.reduce(
                    ...
                    return (
                      <div key={item.month} className="finance-report-year-bar">
                        <div className="finance-report-year-bar-track">
                          <div
                            className="finance-report-year-bar-fill"
                            style={{ height: `${height}%` }}
                          />
                        </div>
                        <span className="finance-report-year-bar-value">
                          {formatCurrency(item.total)}
                        </span>
                        <span className="finance-report-year-bar-label">
