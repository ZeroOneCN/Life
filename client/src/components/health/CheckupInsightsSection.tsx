  ...
  const latestLogs = useMemo(
    () =>
      notificationState.logs
        .filter(
          (log) =>
            log.sceneId === 'checkup.followup_reminder' || log.sceneId === 'checkup.abnormal_alert',
        )
        .slice(0, 5),
    [notificationState.logs],
  );
            ...
            <Row gutter={[16, 16]}>
              <Col span={24}>
                <Field
                  ...
                  onChange={(event) =>
                    onSettingsChange({
                      followUpLeadDays: Math.max(1, Number(event.target.value) || 1),
                    })
                  }
          ...
          <Col span={24}>
            <Row gutter={[16, 16]}>
              {insights.map((insight) => (
                <Col xs={24} sm={12} md={8} lg={6} key={insight.id}>
                  <div className={`checkup-insight-card is-${insight.level}`}>
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          render: (_value, row) => (
                            ...
                            >
                              {row.daysUntilDue < 0
                                ? `已逾期 ${Math.abs(row.daysUntilDue)} 天`
                                : `还有 ${row.daysUntilDue} 天`}
                            </Tag>
                          ),
                        },
                        {
                          ...
                          title: '状态',
                          render: (_value, row) => (
                            <Tag tone={CHECKUP_STATUS_META[row.status].tone}>
                              {CHECKUP_STATUS_META[row.status].label}
                            </Tag>
                          ),
                        },
                      ]}
