...
export function HealthReportAISuggestionSection({
  ...
  return (
    <SectionCard
      title="AI 健康建议"
      description={suggestion ? `生成于 ${suggestion.generatedAt} · ${suggestion.label}` : '基于本期健康数据生成个性化建议'}
      action={
                  ...
                  return (
                    <div key={index} className="health-report-ai-suggestion-item">
                      <div className="health-report-ai-suggestion-header">
                        <Tag tone="default" size="sm">{categoryLabel}</Tag>
                        <Tag tone={tag.tone} size="sm">{tag.text}</Tag>
                      </div>
                      <strong className="health-report-ai-suggestion-title">{item.title}</strong>
