...
export function FitnessDashboardSection({
  ...
  const overview = useMemo(
    ...
    [defaultHeightCm, dietRecords, exerciseRecords, shoppingRecords, weightRecords],
  );
      ...
      <div className="page-stack">
        <StatGrid
          items={[
            ...
            },
            {
              label: '最新体重',
              value:
                overview.latestWeightKg === null ? '-' : `${overview.latestWeightKg.toFixed(2)} kg`,
        ...
        <Row gutter={[24, 20]}>
                  ...
                  <div className="fitness-legend-list">
                    {macroData.map((item, index) => (
                      <div key={item.name} className="fitness-legend-item">
                        <span
                          className="fitness-legend-dot"
                          style={{ background: MACRO_COLORS[index] ?? item.color }}
                        />
                        <span>
                          {item.name}：{item.value.toFixed(1)} g / {item.percentage.toFixed(1)}%
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          <Col span={12}>
            <ChartCard
              title="热量摄入 / 消耗对比"
              description="近 30 天每日总摄入与训练消耗的对比。"
            >
              {hasCalorieTrend ? (
                <div className="fitness-chart-shell">
                  <ResponsiveContainer width="100%" height={280}>
          ...
          <Col span={12}>
            <ChartCard
              title="饮食成本趋势"
              description="根据食材采购价格推算近 30 天每日饮食成本。"
            >
              {hasCostTrend ? (
                <div className="fitness-chart-shell">
                  <ResponsiveContainer width="100%" height={280}>
