  ...
  const latestLogs = useMemo(
    () =>
      notificationState.logs
        .filter(
          (log) =>
            log.sceneId === 'medication.dose_reminder' || log.sceneId === 'medication.stock_low',
        )
        .slice(0, 5),
    [notificationState.logs],
  );
              ...
              <Col span={12}>
                <div className="chart-card">
                  ...
                  <div className="fitness-form-actions">
                    ...
                    <Btn tone="primary" onClick={() => onSaveSummary(selectedDate, summaryText)}>
                      保存每日总结
                    </Btn>
                  </div>
                </div>
              ...
              <Col span={12}>
                <div className="chart-card">
                  ...
                  {monthlySummaries.length ? (
                    <div className="medication-summary-list">
                      {monthlySummaries.map((summary) => (
                        ...
                        >
                          ...
                          <span>
                            {summary.content.replace(/\s+/g, ' ').slice(0, 36) || '无摘要内容'}
                          </span>
                        </button>
                      ))}
                    </div>
                    ...
                    <EmptyState
                      title="本月还没有总结"
                      description="先为一个有用药记录的日期补一段观察笔记。"
                    />
                  )}
                </div>
              </Col>
                    ...
                    <Col span={12}>
                      <div className="fitness-form-actions">
                        <span className="subtle-text">
                          只对购药单位一致的药品做库存估算，不做跨单位换算。
                        </span>
                        <Btn
                          tone="secondary"
                          disabled={!settings.stockReminderEnabled}
                          onClick={onTriggerStockReminder}
                        >
                          执行低库存检查
                        </Btn>
                      </div>
              ...
              <Col span={12}>
                <div className="chart-card">
                  ...
                  {stockInsights.length ? (
                        ...
                        },
                        {
                          ...
                          render: (_value: unknown, row: MedicationStockInsight) =>
                            row.remainingQuantity === null
                              ? '-'
                              : `${row.remainingQuantity} ${row.unit ?? ''}`.trim(),
                        },
                        {
                          key: 'threshold',
                          title: '阈值',
                          render: (_value: unknown, row: MedicationStockInsight) => (
                            <input
                              ...
                              min="0"
                              value={String(
                                settings.medicineThresholds[row.medicineName] ?? row.threshold,
                              )}
                              onChange={(event) =>
                                onSettingsChange({
                                  medicineThresholds: {
                                    ...settings.medicineThresholds,
                                    [row.medicineName]: Math.max(
                                      0,
                    ...
                    <EmptyState
                      title="暂无库存估算数据"
                      description="先录入每日用药和购药记录，库存估算才会开始生效。"
                    />
                  )}
                  {mixedUnitItems.length ? (
                    <div className="callout callout-neutral">
                      {mixedUnitItems.map((item) => item.medicineName).join('、')}{' '}
                      的采购记录单位不一致，当前暂不参与低库存估算。
                    </div>
                  ) : null}
                  {missingPurchaseItems.length ? (
                    <div className="callout callout-neutral">
                      {missingPurchaseItems.map((item) => item.medicineName).join('、')}{' '}
                      尚未找到对应购药记录，当前只做提示不触发库存提醒。
                    </div>
                  ) : null}
                </div>
              </Col>
              ...
              <Col span={12}>
                <div className="chart-card">
                  ...
                  {lowStockItems.length ? (
                    ...
                    <div className="callout callout-neutral">当前没有进入低库存阈值的药品。</div>
                  )}
                </div>
              </Col>
            </Row>
