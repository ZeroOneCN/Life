      ...
      <Row gutter={[12, 12]}>
        <Col span={8}>
          <SettingSwitchCard
            ...
            onChange={(checked) => {
              void savePatch(
                { includeRetiredInDashboard: checked },
                `看板范围已${checked ? '包含' : '排除'}停用物品。`,
              );
            }}
            statusText={settings.includeRetiredInDashboard ? '已开启' : '已关闭'}
            impact={
              settings.includeRetiredInDashboard
                ? '适合做长期总览回看。'
                : '适合只盯当前仍在持续摊销的物品。'
            }
          />
        ...
        <Col span={8}>
          <div className="card switch-card">
            <SelectField
              ...
              onChange={(event) => {
                void savePatch(
                  { defaultSort: event.target.value as StoragePageSettings['defaultSort'] },
                  '默认排序方式已更新。',
                );
              }}
            >
            ...
            <SelectField
              ...
              onChange={(event) => {
                void savePatch(
                  {
                    defaultDashboardRange: event.target
                      .value as StoragePageSettings['defaultDashboardRange'],
                  },
                  '默认看板范围已更新。',
                );
              }}
