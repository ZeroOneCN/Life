      ...
      <div className="page-stack">
        <div className="schedule-settings-grid">
          <SettingSwitchCard
            ...
            onChange={(checked) => {
              void savePatch({ reminderEnabled: checked }, `日程提醒已${checked ? '启用' : '停用'}。`);
            }}
          ...
          >
            <div className="schedule-settings-inline-grid">
              <Field
                ...
                onChange={(event) => {
                  void savePatch({ reminderTime: event.target.value || '08:00' }, '提醒时间已更新。');
                }}
              />
              <SelectField
                label="默认提前提醒"
                ...
                onChange={(event) => {
                  ...
                  if (Number.isFinite(value)) {
                    void savePatch({ defaultReminderMinutes: Math.max(0, value) }, '默认提醒时间已更新。');
                  }
                }}
              >
              ...
              <SelectField
                ...
                onChange={(event) => {
                  void savePatch({ defaultView: event.target.value as ScheduleCalendarView }, '默认日历视图已更新。');
                }}
              >
