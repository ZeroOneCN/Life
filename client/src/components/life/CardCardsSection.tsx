...
interface CardCardsSectionProps {
  ...
  onRecharge: (
    updater: (current: { cards: LifeCardRecord[]; recharges: LifeCardPageState['recharges'] }) => {
      cards: LifeCardRecord[];
...
function parseRechargeDraft(form: RechargeFormState): LifeCardRechargeDraft | null {
  ...
  if (
    ...
    amount <= 0 ||
    !dayjs(form.rechargeDate).isValid()
  ) {
    return null;
  }
...
export function CardCardsSection({
  ...
  const [editingForm, setEditingForm] = useState<CardFormState>(() =>
    createDefaultCardForm(carriers),
  );
  ...
  useEffect(() => {
    if (
      (!form.carrierId || !carriers.some((carrier) => carrier.id === form.carrierId)) &&
      carriers[0]
    ) {
  ...
  const handleManualDeduct = async () => {
    try {
      ...
      if (result.deducted.count > 0) {
        parts.push(
          `已扣账 ${result.deducted.count} 张（合计 ${result.deducted.details.reduce((s, d) => s + Number(d.monthlyFee || 0), 0).toFixed(2)} 元）`,
        );
      }
      if (result.skipped.alreadyCount > 0)
        parts.push(`${result.skipped.alreadyCount} 张本月已扣账`);
      if (result.skipped.billExistsCount > 0)
        parts.push(`${result.skipped.billExistsCount} 张已有本月账单`);
      if (result.skipped.failedCount > 0)
        parts.push(`${result.skipped.failedCount} 张扣账失败，请查看错误详情`);
    ...
    >
      ...
      >
          ...
          <Row gutter={[12, 12]}>
            <Col span={6}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, phoneNumber: event.target.value }))
                }
              />
            </Col>
            <Col span={6}>
              <SelectField
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, carrierId: event.target.value }))
                }
              >
                {carriers.map((carrier) => (
                  <option key={carrier.id} value={carrier.id}>
            ...
            <Col span={6}>
              <DatePickerField
                ...
                onChange={(value) =>
                  setEditingForm((current) => ({ ...current, activationDate: value }))
                }
                clearable={false}
              />
            </Col>
          ...
          <Row gutter={[12, 12]}>
            <Col span={8}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, balance: event.target.value }))
                }
              />
            </Col>
            <Col span={8}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, monthlyFee: event.target.value }))
                }
              />
            </Col>
            <Col span={8}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, billingDay: event.target.value }))
                }
              />
            </Col>
          </Row>
          ...
          <Row gutter={[12, 12]}>
            <Col span={8}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, dataPlan: event.target.value }))
                }
            ...
            <Col span={8}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, callMinutes: event.target.value }))
                }
            ...
            <Col span={8}>
              <Field
                ...
                onChange={(event) =>
                  setEditingForm((current) => ({ ...current, smsCount: event.target.value }))
                }
                placeholder="如 50条/月"
              />
            </Col>
        ...
        <TextArea
          ...
          onChange={(event) =>
            setEditingForm((current) => ({ ...current, notes: event.target.value }))
          }
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Btn tone="primary" onClick={handleRecharge}>
              保存充值
            </Btn>
          </>
        }
      >
        <Row gutter={[12, 12]}>
          ...
          <Col span={8}>
            <Field
              ...
              onChange={(event) =>
                setRechargeForm((current) => ({ ...current, amount: event.target.value }))
              }
              placeholder="0.00"
            />
          </Col>
          <Col span={8}>
            <DatePickerField
              ...
              onChange={(value) =>
                setRechargeForm((current) => ({ ...current, rechargeDate: value }))
              }
              clearable={false}
            />
          </Col>
        ...
        <TextArea
          ...
          onChange={(event) =>
            setRechargeForm((current) => ({ ...current, note: event.target.value }))
          }
