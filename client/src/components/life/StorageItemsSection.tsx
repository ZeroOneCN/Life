...
export function StorageItemsSection({ settings, showToast, onChanged }: StorageItemsSectionProps) {
  ...
  const handleSyncFromShopping = async () => {
    ...
    try {
      ...
      showToast(
        `同步完成：新增 ${result.addedCount} 条，移除 ${result.removedCount} 条，共 ${result.totalCount} 条`,
      );
      onChanged();
      await loadItems();
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        footer={
          <>
            <Btn tone="secondary" onClick={() => setEditingItem(null)}>
              取消
            </Btn>
          ...
          <>
            ...
            <div className="storage-edit-info">
              ...
              <div className="storage-edit-info-item">
                ...
                <strong className="storage-edit-info-value">
                  ¥{formatStorageMoney(editingItem.purchasePrice)}
                </strong>
              </div>
              <div className="storage-edit-info-item">
                <span className="storage-edit-info-label">已使用天数</span>
                <strong className="storage-edit-info-value">
                  {calculateStorageUsageDays(editingItem)} 天
                </strong>
              </div>
              <div className="storage-edit-info-item">
                <span className="storage-edit-info-label">日均成本</span>
                <strong className="storage-edit-info-value storage-edit-daily-cost">
                  ¥{formatStorageMoney(calculateStorageDailyCost(editingItem))}
                </strong>
              </div>
            </div>
            ...
            <Row gutter={[12, 12]}>
              <Col span={8}>
                <Field
                  ...
                  onChange={(event) =>
                    setEditingForm((current) => ({ ...current, itemName: event.target.value }))
                  }
              ...
              <Col span={8}>
                <Field
                  ...
                  onChange={(event) =>
                    setEditingForm((current) => ({ ...current, purchasePrice: event.target.value }))
                  }
                  disabled={editingItem?.source === 'shopping'}
                  placeholder="0.00"
                />
              ...
              <Col span={8}>
                <DatePickerField
                  ...
                  onChange={(value) =>
                    setEditingForm((current) => ({ ...current, purchaseDate: value }))
                  }
                  clearable={false}
                  popoverStrategy="floating"
                  disabled={editingItem?.source === 'shopping'}
              ...
              <Col span={8}>
                <DatePickerField
                  ...
                  onChange={(value) =>
                    setEditingForm((current) => ({ ...current, endDate: value }))
                  }
                  clearable
