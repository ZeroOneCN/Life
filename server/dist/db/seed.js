...
const notificationTemplates = [
    ...
    ['medication.stock_low', '低库存提醒', '你的药品库存已低于提醒阈值，请及时补货。'],
...
const cardCarriers = [
    ...
    ['life-card-carrier-cbn', '中国广电', '适合作为补充性套餐与副号'],
];
...
async function seed() {
    ...
    const sceneChannelRepo = data_source_1.appDataSource.getRepository(notification_center_scene_channel_entity_1.NotificationCenterSceneChannelEntity);
    const carrierRepo = data_source_1.appDataSource.getRepository(life_card_carrier_entity_1.LifeCardCarrierEntity);
    ...
    const defaultPasswordHash = await bcrypt_1.default.hash('12345678', 10);
    await userRepo.save({
        id: defaultUserId,
        username: 'demo',
