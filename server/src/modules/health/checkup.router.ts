...
const recordSchema = z.object({
  ...
  value: z.number(),
  unit: z.string().trim().min(0).max(64),
  referenceRange: z.string().trim().min(0).max(255),
...
function mapRecord(entity: HealthCheckupRecordEntity) {
  return {
    ...
    testDate: dayjs(entity.test_date).format('YYYY-MM-DD'),
    ...
    notes: entity.notes,
    followUpDate: entity.follow_up_date ? dayjs(entity.follow_up_date).format('YYYY-MM-DD') : '',
